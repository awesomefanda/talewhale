...
export function buildUserPrompt(storyIdea, choiceIndex = null, history = []) {
  if (choiceIndex === null && history.length === 0) {
    return `Create the first chapter of a story based on this idea: "${storyIdea}"`;
  }

  const historySummary = history.map((chap, i) =>
    `Chapter ${i + 1}: ${chap.panels?.map(p => p.narration).filter(Boolean).join(' ')}`
  ).join('\n');

  const lastChap = history[history.length - 1];
  // Support both {choice: "text"} (create page) and {choices: [...]} (legacy)
  const chosenPath = lastChap.choice
    ?? lastChap.choices?.[choiceIndex]
    ?? `option ${choiceIndex + 1}`;

  const chapterNum = history.length + 1;
...
Generate chapter ${chapterNum} based on this choice.${chapterNum >= 4 ? ' This is the final chapter — end the story and set "choices" to null.' : ''}
`;
}
